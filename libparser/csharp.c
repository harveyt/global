...
#include "strlimcpy.h"
#include "token.h"
#include "csharp_res.h"

#define MAXCOMPLETENAME 1024            /* max size of complete name of class */
#define MAXCLASSSTACK   100             /* max size of class stack */

/*
 * csharp: read C# file and pickup tag entries.  Almost entirely based on the Java version.
 */
void
csharp(const struct parser_param *param)
{
	int c;
	int level;					/* brace level */
	int startclass, startequal;
	...
	*classname = *completename = 0;
	...
	stack[0].level = 0;
	level = classlevel = 0;
	startclass = startequal = 0;

	if (!opentoken(param->file))
		die("'%s' cannot open.", param->file);

	csharpmode = 1;	/* allow @"..." string literal */

	while ((c = nexttoken(interested, csharp_reserved_word)) != EOF) {
		switch (c) {
		case SYMBOL:					/* symbol */
			for (; c == SYMBOL && peekc(1) == '.'; c = nexttoken(interested, csharp_reserved_word)) {
		...
		case CSHARP_CLASS:
		case CSHARP_STRUCT:
		case CSHARP_INTERFACE:
		case CSHARP_ENUM:
			if ((c = nexttoken(interested, csharp_reserved_word)) == SYMBOL) {
		...
		case CSHARP_IS:
		case CSHARP_AS:
			while ((c = nexttoken(interested, csharp_reserved_word)) == SYMBOL && peekc(1) == '.')
				PUT(PARSER_REF_SYM, token, lineno, sp);
			if (c == SYMBOL)
				PUT(PARSER_REF_SYM, token, lineno, sp);
			break;
		...
		case CSHARP_USHORT:
		case CSHARP_VOID:
			if (peekc(1) == '.' && (c = nexttoken(interested, csharp_reserved_word)) != CSHARP_CLASS)
				pushbacktoken();
			break;
		default:
			break;
